...
function Linux.Create(config)
    ...
    -- Add maximize button
    local MaximizeButton = Linux.Instance("ImageButton", {
        Parent = TopBar,
        BackgroundTransparency = 1,
        Size = UDim2.new(0, 16, 0, 16),
        Position = UDim2.new(1, -46, 0.5, -8),
        Image = "rbxassetid://10734886496",
        ImageColor3 = Color3.fromRGB(180, 180, 190),
        ZIndex = 3
    })
    ...
    -- Maximize button functionality (placeholder)
    MaximizeButton.MouseEnter:Connect(function()
        TweenService:Create(MaximizeButton, TweenInfo.new(0.2), {ImageColor3 = Color3.fromRGB(220, 220, 230)}):Play()
    end)
    
    MaximizeButton.MouseLeave:Connect(function()
        TweenService:Create(MaximizeButton, TweenInfo.new(0.2), {ImageColor3 = Color3.fromRGB(180, 180, 190)}):Play()
    end)
    ...
    function LinuxLib.Tab(config)
        ...
        function TabElements.Button(config)
            ...
            local accentColor = Color3.fromRGB(70, 90, 120)
            ...
            local function UpdateToggle(thisToggle)
                ...
                if thisTrack and thisKnob and thisText then
                    ...
                    if currentState then
                        local color1 = Color3.fromRGB(70, 90, 120)
                        local color2 = Color3.fromRGB(100, 120, 160)
        ...
        function TabElements.Dropdown(config)
            ...
            local Selected = Linux.Instance("TextLabel", {
                ...
                TextColor3 = Color3.fromRGB(70, 90, 120),
                TextSize = 14,
                ...
                if IsOpen then
                    for i, opt in pairs(Options) do
                        local OptBtn = Linux.Instance("TextButton", {
                            ...
                            Font = Enum.Font.GothamSemibold,
                            Text = tostring(opt),
                            TextColor3 = opt == SelectedValue and Color3.fromRGB(70, 90, 120) or Color3.fromRGB(200, 200, 210),
                            TextSize = 14,
                            ZIndex = 3,
                            AutoButtonColor = false,
                            LayoutOrder = i
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            if opt ~= SelectedValue then
                                ...
                                Selected.TextColor3 = Color3.fromRGB(70, 90, 120)
                                
                                for _, btn in pairs(OptionsHolder:GetChildren()) do
                                    if btn:IsA("TextButton") then
                                        if btn.Text == tostring(opt) then
                                            btn.TextColor3 = Color3.fromRGB(70, 90, 120)
        ...
        function TabElements.Colorpicker(config)
            ...
            local function CreateColorpickerPopup()
                ...
                -- Cria um gradiente horizontal para saturação
                ...
                -- Cria uma sobreposição para valor (brilho)
                ...
                -- Adiciona um gradiente vertical para valor (brilho)
                local ValueGradient = Linux.Instance("UIGradient", {
                    ...
                    Transparency = NumberSequence.new({
                        NumberSequenceKeypoint.new(0, 1), -- Totalmente transparente no topo (brilho máximo)
                        NumberSequenceKeypoint.new(1, 0)  -- Totalmente opaco embaixo (brilho mínimo)
                    }),
                    Rotation = 90
                })
                ...
                -- Adiciona um gradiente para representar matiz (hue)
                local HueGradient = Linux.Instance("UIGradient", {
                    Parent = HueBar,
                    Color = ColorSequence.new({
                        ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 0, 0)),   -- Vermelho (0 graus)
                        ColorSequenceKeypoint.new(0.167, Color3.fromRGB(255, 255, 0)), -- Amarelo (60 graus)
                        ColorSequenceKeypoint.new(0.333, Color3.fromRGB(0, 255, 0)),   -- Verde (120 graus)
                        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(0, 255, 255)),   -- Ciano (180 graus)
                        ColorSequenceKeypoint.new(0.667, Color3.fromRGB(0, 0, 255)),   -- Azul (240 graus)
                        ColorSequenceKeypoint.new(0.833, Color3.fromRGB(255, 0, 255)), -- Magenta (300 graus)
                        ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 0, 0))    -- Vermelho (360 graus)
                    }),
                    Rotation = 90
                })
